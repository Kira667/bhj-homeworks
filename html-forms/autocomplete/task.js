class Autocomplete {
  ...
  getMatches( text ) {
		const optionNodes = Array.from(this.input.querySelectorAll('option'));
		const optionNodesInfo = optionNodes.map((item, i) => ({
			value: item.value,
			text: item.text
		}));
		const filteredOptionNodesInfo = optionNodesInfo.filter(item => {
			return item.text.toLowerCase().includes(text.toLowerCase());
		});

    return filteredOptionNodesInfo;
  }
}
